...
class purchaseOrderDetailPage extends ConsumerStatefulWidget {
  const purchaseOrderDetailPage({super.key, required this.title, required this.id});
  final String title;
  final String id;
  

  @override
  ConsumerState<purchaseOrderDetailPage> createState() => _PurchaseOrderPageState();
  
}

class _PurchaseOrderPageState extends ConsumerState<purchaseOrderDetailPage> {
  List<Map<String,dynamic>> myList = [];
  Map<String, dynamic> saveItems = {};
  Map<String, dynamic> statusItems = {};
  List<dynamic> profDet = [];
  // List<Map<String,dynamic>> myListParts = [];
  ...
  void initState(){
    ...
    // final POParts pc = POParts();
     Future.delayed(Duration.zero, () async {
      ...
      // List<Map<String,dynamic>> result2 = await pc.createList(widget.id);
      setState(() {
        myList = result; 
        roleList = role;
        profDet = profile;
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            Padding(
                                                padding: EdgeInsets.only(top: h * 0.01, left: w * 0.01),
                                                child: Text(
                                                  'Discount(%):  '+ formatter.format(disc),
                                                  style: TextStyle(fontSize: h * 0.02, color: Colors.black), 
                                                ),
                                              ),
                                          ],
